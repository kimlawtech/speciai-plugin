...
### /speciai-advisory:legaldoc — 법률문서 30여 유형

```
> 거래처가 물품대금 3,300만원을 안 줘. 소장 써줘
> 투자 받는데 주주간계약서 초안 필요해 (SAFE 아니고 우선주)
> 편의점 가맹계약 해지 통고서
> 과징금 처분 행정심판 청구서
```

웹 DocumentReview "AI 작성"과 동일한 문서 엔진을 CLI에서 쓴다 — 문서군(계약서/소송·신청/통지문/진술) 자동 판정, 유형별 필수·주의 조항 30세트, 업종 특약, 공정위 표준계약서 119종·대한법률구조공단 작성례 958건 참조. 관할·사건번호처럼 모르는 값은 채우지 않고 "확인이 필요한 사항"으로 남기며, 인용 조문은 전수 검증한다. 출력은 A4 DOCX + 제출용 HWPX.

### /speciai-advisory:jangbu — 장부·경비 관리

```
> 이 폴더에 있는 카드내역이랑 통장내역 장부로 정리해줘
```

은행 CSV(KB·신한·우리·하나·IBK 자동 탐지, EUC-KR 대응)·카드사 이용내역 CSV·카드명세서 PDF를 읽어 원장을 만들고, 한국 거래처 분류 룰(스타벅스→복리후생비, 배민 5만원 기준 접대비 분기 등)로 자동 분류한다. 미분류 건만 대화로 확정하면 손익 리포트·국세청 표준계정 코드가 붙은 세무사 전달 CSV까지 나온다. 원장은 `~/.speciai/jangbu/` 로컬 전용 — 거래 내역은 서버로 전송되지 않는다.

### /speciai-advisory:receipt — 증빙 OCR

영수증·세금계산서·카드매출전표 사진이나 PDF를 첨부하면 판독해 구조화하고, 공급자 사업자번호를 국세청에서 실시간 검증(휴·폐업이면 매입세액 불공제 경고)한 뒤 장부에 넣는다.

### /speciai-advisory:certmail — 내용증명 (14유형)

임금체불·보증금·대여금·환불 등 유형을 자동 분기해 우체국 제출 규격 DOCX를 생성한다. 법령 인용은 전수 검증.

### /speciai-advisory:privacy — 개인정보처리방침·이용약관

2026.3 개정 개인정보 보호법 기준 필수 기재사항 체크리스트로 인터뷰 후 생성.

### 거래처 검증 (speciai_biz_status)

```
> 사업자번호 192-34-01701 이 거래처 정상 사업자야?
→ 계속사업자 · 부가가치세 일반과세자 (국세청 실시간)
```
...
| 도구 | 용도 | 예시 지시 |
|---|---|---|
| speciai_doc_guide | 문서 유형별 작성 규칙·참조 자산 | "가압류 신청서 필수 항목 뭐야" |
| speciai_biz_status | 국세청 사업자 상태·과세유형 | "이 사업자번호 폐업 아닌지 확인해줘" |
